...
impl<U: Deref, const D: usize> MirrorExt<D> for U
where
    U::Target: MirrorExt<D>,
{
    fn append_intersecting_points(&self, ray: &Ray<D>, list: &mut Vec<Tangent<D>>) {
...
pub trait JsonSer {
    fn ser(&self) -> Result<serde_json::Value, Box<dyn Error>>;
}
...
trait JsonSerDyn: JsonTypeDyn + JsonSer {}

impl<T: JsonTypeDyn + JsonSer + ?Sized> JsonSerDyn for T {}
...
pub trait OpenGLRenderable {
    fn render(&self, display: &gl::Display) -> Vec<Box<dyn render::RenderData>>;
}
...
{
    fn render(&self, display: &gl::Display) -> Vec<Box<dyn render::RenderData>> {
        self.deref().render(display)
    }
}

/// wrapper struct that exists as a hack around the lack of trait
/// implementation disjointness detection based on associated types
...
impl<T: JsonSerDyn> JsonSer for Dynamic<T> {
    fn ser(&self) -> Result<serde_json::Value, Box<dyn Error>> {
        Ok(serde_json::json!({
